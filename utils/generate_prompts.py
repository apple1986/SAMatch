...
def get_click_prompt(datapack, opt):
    ...
    return pt
import numpy as np
def get_click_prompt_1(pt,point_labels):
    pt = np.array(pt)
    point_coords = pt
    coords_torch = torch.as_tensor(point_coords, dtype=torch.float32, device=('cuda:0'))
    labels_torch = torch.as_tensor(point_labels, dtype=torch.int, device=('cuda:0'))
    if len(pt.shape) == 2:
        coords_torch, labels_torch = coords_torch[None, :, :], labels_torch[None, :]
    pt = (coords_torch, labels_torch)
    return pt
