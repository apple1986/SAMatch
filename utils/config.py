...
class Config_MRliver:
    # This dataset is for breast cancer segmentation
    root_path = "/home/gxu/proj1/smatch"
    data_path = root_path + "/data/MRliver"
    data_subpath = "./MRliver"
    model_tyep = "Unimatch_samus_F2_iter3000_5_labeled_bs4/unet_drop"  # SAMUS_Point  SAMUS_Point_Box, SAMUS_Point_case3
    save_path = root_path + "/checkpoint/MRliver/" + model_tyep
    result_path = os.path.join(save_path, "result") 
    tensorboard_path = os.path.join(save_path, "tensorboard")
    load_path = os.path.join(save_path, "best_sam_model.pth")
    save_path_code = "_"
    ...
    img_size = 256                      # the input size of model
    train_split = "train"   # the file name of training set
    val_split = "val"       # the file name of validation set
    test_split = "test"     # the file name of testing set
    crop = None                         # the cropped image size
    eval_freq = 1                       # the frequency of evaluate the model
    save_freq = 200                    # the frequency of saving the model
    device = "cuda"                     # training device, cpu or cuda
    cuda = "on"                         # switch on/off cuda option (default: off)
    gray = "yes"                        # the type of input image
    img_channel = 1                     # the channel of input image
