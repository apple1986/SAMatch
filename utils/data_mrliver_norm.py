...
class JointTransform2D:
    ...
    def __call__(self, image, mask):
        ...
        if np.random.rand() < self.p_gama:
            ...
            image = (np.power(image, 1.0 / g) / c) #* 255
...
class ImageToImage2D(Dataset):
    ...
    def __init__(self, dataset_path: str, split='train', joint_transform: Callable = None, img_size=256, prompt = "click", class_id=1,
                 one_hot_mask: int = False, cases="all") -> None:
        self.dataset_path = dataset_path
        self.one_hot_mask = one_hot_mask
        self.split = split
        if cases=="all":
            id_list_file = os.path.join(dataset_path, '{0}_slices.list'.format(split))
        else:
            id_list_file = os.path.join(dataset_path, '{0}_slices_{1}.list'.format(split, cases))
    ...
    def __getitem__(self, i):
        id_ = self.ids[i]
        # if self.split == "train":
        h5f = h5py.File(os.path.join(self.dataset_path, "slices/{}.h5".format(id_)), "r")
        # else:
        #     h5f = h5py.File(self.dataset_path + "volumes/{}.h5".format(id_), "r")
        image = h5f["image"][:]
        mask = h5f["label"][:]
        ...
        if self.prompt == 'click':
            ...
            if 'train' in self.split:
                pt, point_label = random_click(np.array(mask), class_id) # pt: x,y  == column, row 
