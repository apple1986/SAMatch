...
def get_bbox256( mask_256, bbox_shift=3):
    ...
    """
    if mask_256.sum() > 10:
        # print(mask_256.shape)
        # print(np.where(mask_256 > 0))
        ...
        bboxes256 = np.array([-100, -100, -100, -100]) # shape: (4, )

    return bboxes256

def get_multi_bbox256(gt_mask): # there are 4 class: 0:bg, 1:RV, 2:MYO, 3:LV
    bbox_all = np.zeros((3, 4))
    for num in range(1, 4):
        gt_mask_bin = (gt_mask == num)#.astype("uint8")
        bbox_all[num-1, :] = get_bbox256(gt_mask_bin, bbox_shift=3) # shape: 3x4, Nx4
...
class BaseDataSets(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.split == "train":
            ...
            sample['image'] =  image #resize_val(image)
            sample['label'] = label #resize_val(label)
        sample["idx"] = idx # image: 1x256x256, label: 256 x 256
 
        # get the box from gt
        sample['bbox'] = get_multi_bbox256(sample['label']) # shape: 3X4
...
class RandomGenerator(object):
    ...
    def __call__(self, sample):
        ...
        # label = lab[ind, ...]
        if random.random() > 0.5:
            image, label = random_rot_flip(image, label) # rotation 90 and flip
        elif random.random() > 0.5:
            image, label = random_rotate(image, label) # roataion [-20, 20]
        x, y = image.shape
...
class WeakStrongAugment(object):
    ...
    weakly aug: change a little of an image; strong aug: change much of an image
    Args:
