...
#  =========================================== parameters setting ==================================================
MODEL_PATH = "Unimatch_samus_F2_iter3000_5_labeled_bs4"
...
parser.add_argument('--matchnet', type=str, default='/home/gxu/proj1/smatch/checkpoint/MRliver/'+MODEL_PATH+'/unet_drop/unet_drop_best_model.pth', help='the path of MatchNet')
parser.add_argument('--sam_path', type=str, default='/home/gxu/proj1/smatch/checkpoint/MRliver/'+MODEL_PATH+'/unet_drop/best_sam_model.pth', help='the path of MatchNet')
...
def main():
    ...
    model.to(device)
    # load sam
    checkpoint = torch.load(args.sam_path, map_location='cuda:0', weights_only=True)
...
#  ========================================================================= begin to evaluate the model ============================================================================
    ...
    for case in tqdm(image_list):
        first_metric = test_single_volume(
            case, model, matchnet, opt.result_path, opt)
...
def load_matchnet(net_path):
    # build model
    matchnet = net_factory(net_type="unet_drop", in_chns=1, class_num=2)
    # load trained parameters
    # matchnet.load_state_dict(torch.load(net_path, map_location='cuda:0', weights_only=True)["state_dict"])
    matchnet.load_state_dict(torch.load(net_path, map_location='cuda:0', weights_only=False)["model"])
...
#############################: test each volume
def test_single_volume(case, net, matchnet, test_save_path, FLAGS):
    ...
    for ind in range(0, image.shape[2]):
        ...
        out_prob_obj = out_prob[:,1,:,:] # BHW
        points_coord, points_label = find_point_label_pseudo(out_prob_obj)
        point_label = (points_coord, points_label)
...
if __name__ == '__main__':
    ...
    with open(os.path.join(args.save_path, "eval_sam_point_match.txt"),'w') as f:
