...
parser.add_argument("--exp", type=str, default="MRliver/FixMatch_samus_F2", help="experiment_name")
...
parser.add_argument('--ckpt_samus', type=str, default='/home/gxu/proj1/smatch/checkpoint/MRliver/SAMUS_Point_case3-exp1-seed-2024/SAMUS_09251451_269_0.8669143485606119.pth', help='Pretrained checkpoint of SAM')
...
def train(args, snapshot_path):
    ...
    optimizer_sam = optim.SGD(sam.parameters(), lr=base_lr*0.01, momentum=0.9, weight_decay=0.0001)
    ...
    for epoch_num in iterator:
        ...
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            ## pred from sam
            # obtain prompt
            all_pd_stu = outputs_weak_masked[:, 1,:,:] # probality
            point_label = find_position(all_pd_stu)
            # predicition from samus
            ## notice: you should norm sam before inputting
            ...
            # out_main = sam(img_sam, point_label)
            predict = torch.sigmoid(out_main['masks']) # predict: B4HW
            sam_out = predict[:, 0, :, :] > 0.5  # (b, h, w)  
            sam_out = sam_out[args.batch_size//2:].long() # for unlabeled data as gt : BHW
            ...
            ## training samus : mask = F.resize(image, (new_h, new_w), InterpolationMode.BILINEAR)
            ...
            loss_sam = loss_sam_unlabel + loss_sam_label

            ## student model update
            loss = sup_loss + consistency_weight * unsup_loss_1
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
            if iter_num > 25000:
