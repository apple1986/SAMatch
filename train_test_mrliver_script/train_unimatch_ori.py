...
from networks.unet_unimatch import UNet
from utils.utils import AverageMeter, count_params, init_log, DiceLoss


CLASSES = {'MRliver': ['liver']}

parser = argparse.ArgumentParser(description='Revisiting Weak-to-Strong Consistency in Semi-Supervised Semantic Segmentation')
parser.add_argument('--config', type=str, default='./configs/mrliver.yaml',  required=False)
...
parser.add_argument('--save_path', type=str, default="./checkpoint/MRliver/UniMatch/case3", required=False)
...
def main():
    ...
    rank, world_size = 0, 1 #setup_distributed(port=args.port)
    ...
    trainset_u = MRliverDataset(cfg['dataset'], cfg['data_root'], 'train_u',
                             cfg['crop_size'], args.unlabeled_id_path)
    trainset_l = MRliverDataset(cfg['dataset'], cfg['data_root'], 'train_l',
                             cfg['crop_size'], args.labeled_id_path, nsample=len(trainset_u.ids))
    valset = MRliverDataset(cfg['dataset'], cfg['data_root'], 'val')
