...
from utils.get_prompts import find_box

parser = argparse.ArgumentParser()
parser.add_argument("--root_path", type=str, default="./data/MRliver", help="Name of Experiment")
parser.add_argument("--exp", type=str, default="MRliver/FixMatch_sam_pseudo_label", help="experiment_name")
...
parser.add_argument("--labeled_num", type=int, default=9, help="labeled number of cases")
# costs
...
def patients_to_slices(dataset, patiens_num):
    ...
    if "ACDC" in dataset:
        ...
        ref_dict = {"1": 64, "3": 192, "7": 448, "9": 576, "11": 704, "30": 1920}
    else:
        print("Error")
...
def train(args, snapshot_path):
    ...
    sam = sam_model_registry["vit_b"](checkpoint="./checkpoint/SAM/sam_vit_b_01ec64.pth").to(cuda_num)
    ...
    for epoch_num in iterator:
        ...
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            pseudo_outputs = torch.argmax(outputs_weak_masked[args.labeled_bs :].detach(), dim=1, keepdim=False) #BHW

            consistency_weight = get_current_consistency_weight(iter_num // 150)

            # output pseudo-label by sam
            img_sam = F.interpolate(weak_batch[args.labeled_bs :], size=(1024,1024)) #BxCx1024x1024
            img_sam = torch.cat((img_sam, img_sam, img_sam), dim=1) # B3HW
            
            # generate mask from sam
            pseudo_outputs_sam = torch.zeros_like(pseudo_outputs)
            masks_pd = pseudo_outputs.cpu().numpy().astype("uint8")
            for n in range(args.batch_size-args.labeled_bs): # unlabeled data
                # get box
                if (masks_pd[n,:,:].sum()) > 0 and iter_num > 10000:
                    input_box = find_box(masks_pd[n,:,:]) 
                    input_box = predictor.transform.apply_boxes(input_box, pseudo_outputs.shape[1:])
                    input_box = torch.tensor(input_box).unsqueeze(0).to(cuda_num) # Bx4

                    predictor.set_torch_image(img_sam[n, :,:,:].unsqueeze(0), weak_batch.shape[2:])
                    mask, _, _ = predictor.predict_torch(
                                                    point_coords = None,
                                                    point_labels=[1],
                                                    # mask_input=pseudo_outputs[n, :,:].float().unsqueeze(0),
                                                    boxes=input_box,
                                                    multimask_output=False,
                                                        )
                    pseudo_outputs_sam[n,:,:] = mask.detach().long() # CHW
                else:
                    # print("check: ", pseudo_outputs[n,:,:].sum())
                    pseudo_outputs_sam[n,:,:] = pseudo_outputs[n,:,:].detach()# CHW
            # supervised loss
            ...
            # unsupervised loss
            unsup_loss = (
                ce_loss(outputs_strong[args.labeled_bs :], pseudo_outputs_sam)
                + dice_loss(outputs_strong_soft[args.labeled_bs :], pseudo_outputs_sam.unsqueeze(1))
                + as_weight * comp_loss
            )
