## get the prompts (point and box) from the GT
import argparse
import os
import shutil
import matplotlib
...
def test_single_volume(case, net, predictor, test_save_path, FLAGS):
    ...
    for ind in range(0, image.shape[2]):
        ...
        with torch.no_grad():
            ...
            # input_points = torch.tensor([[45, 150], [43, 150], [10,10]]).cuda()
            # input_box = torch.tensor([[26, 134, 60, 165]]).cuda()

            # get the box from gt
            if gt.sum() > 0:
                ## get point
                input_points = find_centroid(gt)
                # input_points = np.array([input_points[0,:], [200,150]])
                input_points = predictor.transform.apply_coords(input_points, gt.shape)
                input_points = torch.tensor(input_points).unsqueeze(0).cuda()

                ## get box
                input_box = find_box(gt) 
                input_box = predictor.transform.apply_boxes(input_box, gt.shape)
                input_box = torch.tensor(input_box).unsqueeze(0).cuda() # Bx4
                ...
                mask, _, _ = predictor.predict_torch(
                                                point_coords = input_points, # BxNx2
                                                # point_labels=torch.tensor([[1, 0]]).cuda(), # BxN 
                                                # point_coords = None,
                                                point_labels=torch.tensor([[1]]).cuda(), # BxN 
...
if __name__ == '__main__':
    ...
    save_res_path = "./checkpoint/{}_{}_labeled_bs{}/test_metrics_prompt_centroid_box.txt".format(
