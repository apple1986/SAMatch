...
parser.add_argument("--exp", type=str, default="MRliver/Unimatch_medsam_F2_iter30000_bbox30_lr_1e_2_5e_4", help="experiment_name")
parser.add_argument("--iter_num", type=int, default=30000, help="when train sam after matchnet")
parser.add_argument("--cuda_num", type=str, default="cuda:1", help="experiment_name")
parser.add_argument("--bbox_shift", type=int, default=30, help="bbox shift")
parser.add_argument("--labeled_num", type=int, default=LABELED_NUM_USED, help="labeled data")
...
parser.add_argument('--ckpt_sam', type=str, 
                    default='/home/gxu/proj1/smatch/checkpoint/MRliver/MedSam_ft_'+str(LABELED_NUM_USED)+'_labeled_bs4/medsam/medsam_lite_best.pth', 
                    help='Fine-tuned SAM')
parser.add_argument('--ckpt_uni', type=str, 
                    default='/home/gxu/proj1/smatch/checkpoint/MRliver/Unimatch_'+str(LABELED_NUM_USED)+'_labeled_bs4/unet_drop/unet_drop_best_model.pth', 
                    help='Fine-tuned SAM')
...
def load_match(ckpt_match, model):
    load_weight = torch.load(ckpt_match, weights_only=False)["model"]
...
def train(args, snapshot_path):
    ...
    for epoch_num in iterator:
        ...
        for i, ((img_x, mask_x),
            ...
            # get box from gt mask
            img_gt_bbox = get_bbox256_torch(mask_x) # Bx1x4
            # get box from match pd mask
            img_pd_bbox = get_bbox256_cv(mask_u_w, bbox_shift=args.bbox_shift)
            img_bbox_all = torch.cat((img_gt_bbox, img_pd_bbox), dim=0)
            medsam_logit, iou_pred = sam(img_sam, img_bbox_all)
            medsam_mask = torch.sigmoid(medsam_logit) > 0.5 # 8x1xHxW
            # medsam_logit = medsam_logit.squeeze(1)

            mask_u_w_sam = medsam_mask[num_lb:].squeeze(1).long() # for unlabeled data as 'gt' mask : BHW
            pred_sam_pos = torch.sigmoid(medsam_logit) # B*1*H*W
            ...
            loss_x_sam = (ce_loss(pred_x_sam, mask_x) + 
                        dice_loss(pred_x_sam, mask_x.unsqueeze(1).float())) / 2.0
