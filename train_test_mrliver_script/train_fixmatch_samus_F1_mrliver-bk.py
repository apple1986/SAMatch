...
parser.add_argument("--exp", type=str, default="MRliver/FixMatch_samus_F1", help="experiment_name")
...
parser.add_argument("--labeled_num", type=int, default=3, help="labeled number of cases used for training")
...
parser.add_argument('--ckpt_samus', type=str, default='/home/gxu/proj1/smatch/checkpoint/MRliver/SAMUS_Point_case3/SAMUS_09251451_269_0.8669143485606119.pth', help='Pretrained checkpoint of SAM')
...
def train(args, snapshot_path):
    ...
    for epoch_num in iterator:
        ...
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            ## get the position of the highest confident point
            unlabel_pd = outputs_weak_masked[args.labeled_bs:, 1,:,:]
            point_label = find_position(unlabel_pd)
            # B, H, W = unlabel_pd.shape            
            # M = unlabel_pd.view(B, -1).argmax(1)
            # idx = torch.cat(((M / H).view(-1, 1), (M % W).view(-1, 1)), dim=1).long()
            # idx[:, [0,1]] = idx[:, [1,0]]
            # input_points = idx.unsqueeze(1)
            # input_labels = torch.ones((B, 1), device=args.cuda_num)
            ...
            # # sam_mask = torch.argmax(torch.softmax(
            # #     outputs_weak_masked[args.labeled_bs :], dim=1), dim=1)
            ...
            out_main = sam(F.normalize(img_sam), point_label, bbox=None) # 
            predict = torch.sigmoid(out_main['masks'])
            predict = predict.detach()  # (b, c, h, w)
            pseudo_outputs_sam = predict[:, 0, :, :] > 0.5  # (b, h, w)  
            pseudo_outputs_sam = pseudo_outputs_sam.long()
            # supervised loss
