...
from utils.data_mrliver_norm import JointTransform2D, ImageToImage2D
...
def main():
    ...
    parser.add_argument('--cases', type=str, default="case5", help='how many cases involved in training, choice: all, case3, case7, case9')
    ...
    ## get all hyper-parameters setting
    ...
    if args.keep_log:
        ...
        boardpath = os.path.join(opt.tensorboard_path, logtimestr)
        if not os.path.isdir(boardpath):
            os.makedirs(boardpath) # save training log
        TensorWriter = SummaryWriter(boardpath)

    #  =============================================================== add the seed to make sure the results are reproducible ==============================================================

    seed_value = 1234 #2024  # the number of seed
    ...
    if not os.path.exists(opt.tensorboard_path):
        ...
        os.makedirs(opt.tensorboard_path)

    # register the sam model
    model = get_model(args.modelname, args=args, opt=opt) # obtain the model architecture
    ...
    train_dataset = ImageToImage2D(opt.data_path, opt.train_split, tf_train, 
                                   img_size=args.encoder_input_size, cases=args.cases)
    ...
    model.to(device)
    if opt.pre_trained:
        checkpoint = torch.load(opt.load_path, weights_only=True)
        new_state_dict = {}
    ...
    for epoch in range(opt.epochs):
        ...
        for batch_idx, (datapack) in enumerate(trainloader):
            ...
            pred['low_res_logits'] = pred['low_res_logits'][:,0,:,:].unsqueeze(1) # torch.Size([8, 1, 128, 128])
            train_loss = criterion(pred, masks) #  torch.Size([8, 1, 128, 128])
            # -------------------------------------------------------- backward -------------------------------------------------------
            optimizer.zero_grad()
            train_loss.backward()
            optimizer.step()
        ...
        if epoch % opt.eval_freq == 0:
            ...
            if mean_dice > best_dice:
                ...
                if not os.path.isdir(opt.save_path):
                    os.makedirs(opt.save_path)
                save_path = os.path.join(opt.save_path, args.modelname + opt.save_path_code + '%s' % timestr + '_' + str(epoch) + '_' + str(best_dice))
        ...
        if epoch % opt.save_freq == 0 or epoch == (opt.epochs-1):
            if not os.path.isdir(opt.save_path):
                os.makedirs(opt.save_path)
            save_path = os.path.join(opt.save_path, args.modelname + opt.save_path_code + str(epoch))
            torch.save(model.state_dict(), save_path + ".pth", _use_new_zipfile_serialization=False)
            if args.keep_log:
                with open(os.path.join(opt.tensorboard_path, logtimestr + '/trainloss.txt'), 'w') as f:
                    for i in range(len(loss_log)):
                        f.write(str(loss_log[i])+'\n')
                with open(os.path.join(opt.tensorboard_path, logtimestr + '/dice.txt'), 'w') as f:
