...
parser.add_argument("--exp", type=str, default="MRliver/Unimatch_samus_F2_iter4000_prob_sam", help="experiment_name")
parser.add_argument("--iter_num", type=int, default=4000, help="when train sam after matchnet")
...
parser.add_argument('--ckpt_samus', type=str, 
                    default='/home/gxu/proj1/smatch/checkpoint/MRliver/SAMUS_Point_Norm_case3/SAMUS_10012349_369_0.8747810116403715.pth', 
                    help='Fine-tuned SAM')
    ...
    for epoch_num in iterator:
        ...
        for i, ((img_x, mask_x),
            ...
            if iter_num < args.iter_num: # only update unimatch           
                ...
                point_coord_pd, point_label_pd = find_point_label_pseudo(mask_u_w)
                ...
                loss_u_w_fp = dice_loss(pred_u_w_fp.softmax(dim=1), 
                                        mask_u_w_sam.unsqueeze(1).float(),
                                            ignore=(conf_u_w_sam < args.conf_thresh).float())
