...
def test_single_volume_synapse(image, label, net, classes, patch_size=[256, 256], cuda_num="cuda:0"):
    image, label = image.squeeze(0).cpu().detach(
    ).numpy(), label.squeeze(0).cpu().detach().numpy()
    prediction = np.zeros_like(label)
...
def test_single_volume_ds(image, label, net, classes, patch_size=[256, 256], cuda_num="cuda:0"):
    image, label = image.squeeze(0).cpu().detach(
    ).numpy(), label.squeeze(0).cpu().detach().numpy()
    prediction = np.zeros_like(label)
    for ind in range(image.shape[0]):
