    ...
    if threeD:
        ...
        network = Generic_UNet(default_dict["num_input_channels"], default_dict["base_num_features"], default_dict["num_classes"], len(default_dict["net_num_pool_op_kernel_sizes"]),
                            ...
                            net_nonlin, net_nonlin_kwargs, False, False, lambda x: x, InitWeights_He(1e-2),
                                ...
                                default_dict["net_conv_kernel_sizes"], False, True, True)
    else:
        conv_op = nn.Conv2d
        ...
        net_nonlin = nn.LeakyReLU
        net_nonlin_kwargs = {'negative_slope': 1e-2, 'inplace': True}
        default_dict["net_num_pool_op_kernel_sizes"] = [[2, 2], [2, 2], [2, 2], [2, 2], [2, 2], [2, 2]]
        default_dict["net_conv_kernel_sizes"] = [[3, 3], [3, 3], [3, 3], [3, 3], [3, 3], [3, 3], [3, 3]]
...
if __name__ == "__main__":
    input = torch.FloatTensor(1, 1, 192, 192)
    input_var = input.cuda()
    model = initialize_network(threeD=False)
    out = model(input_var)
    print(out.size())
