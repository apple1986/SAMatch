...
def _build_samus(
    ...
    samus = Samus(
        ...
        prompt_encoder=PromptEncoder( # Encodes prompts for input to SAM's mask decoder.
    ...
    if checkpoint is not None:
        with open(checkpoint, "rb") as f:
            state_dict = torch.load(f, weights_only=True) # load pretrained sam weight
        try:
            samus.load_state_dict(state_dict)
        except:
            new_state_dict = load_from2(samus, state_dict, image_size, patch_size) # put part sam weight into samus
